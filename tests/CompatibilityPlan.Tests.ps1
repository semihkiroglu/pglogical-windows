...
function New-Result {
    param(
        ...
        [string]$PackageTag = "pglogical-2.4.8-pg$Major-windows.1",
...
Test-Case 'Latest release selection is per major and ignores the global latest major' {
    $releases = @(
        (New-Release -Tag 'pglogical-2.4.8-pg14-windows.1' -ArtifactFilename 'postgresql-14.23-1-windows-x64-binaries.zip'),
        (New-Release -Tag 'pglogical-2.4.8-pg14-windows.2' -ArtifactFilename 'postgresql-14.23-2-windows-x64-binaries.zip'),
        (New-Release -Tag 'pglogical-2.4.9-pg14-windows.1' -ArtifactFilename 'postgresql-14.24-1-windows-x64-binaries.zip'),
        (New-Release -Tag 'pglogical-2.5.0-pg18-windows.1' -ArtifactFilename 'postgresql-18.6-2-windows-x64-binaries.zip')
    )
...
Test-Case 'Equal upstream versions select the highest packaging revision' {
    $releases = @(
        (New-Release -Tag 'pglogical-2.4.8-pg18-windows.1' -ArtifactFilename 'postgresql-18.6-2-windows-x64-binaries.zip'),
        (New-Release -Tag 'pglogical-2.4.8-pg18-windows.3' -ArtifactFilename 'postgresql-18.6-2-windows-x64-binaries.zip'),
        (New-Release -Tag 'pglogical-2.4.8-pg18-windows.2' -ArtifactFilename 'postgresql-18.6-2-windows-x64-binaries.zip')
    )
    Assert-Equal 'pglogical-2.4.8-pg18-windows.3' (Select-LatestReleaseForMajor -Major '18' -Releases $releases).tag_name
}

Test-Case 'Smoke plan marks drift as test, matching artifact as covered, and missing package as pending' {
    $releases = @(
        (New-Release -Tag 'pglogical-2.4.8-pg14-windows.1' -ArtifactFilename 'postgresql-14.23-1-windows-x64-binaries.zip'),
        (New-Release -Tag 'pglogical-2.4.8-pg15-windows.1' -ArtifactFilename 'postgresql-15.19-2-windows-x64-binaries.zip')
    )
    ...
    Assert-Equal 'pglogical-2.4.8-pg14-windows.1' (@($plan | Where-Object postgresqlMajor -eq '14')[0].localReleaseTag)
}
...
Test-Case 'Targeted rebuild plans only the failed major with the next revision' {
    $local = @(
        (New-Release -Tag 'pglogical-2.4.8-pg17-windows.1' -ArtifactFilename 'postgresql-17.11-1-windows-x64-binaries.zip'),
        (New-Release -Tag 'pglogical-2.4.8-pg18-windows.1' -ArtifactFilename 'postgresql-18.4-1-windows-x64-binaries.zip')
    )
...
Test-Case 'Targeted rebuild uses highest existing revision plus one even with gaps' {
    $local = @(
        (New-Release -Tag 'pglogical-2.4.8-pg18-windows.1' -ArtifactFilename 'postgresql-18.4-1-windows-x64-binaries.zip'),
        (New-Release -Tag 'pglogical-2.4.8-pg18-windows.3' -ArtifactFilename 'postgresql-18.5-1-windows-x64-binaries.zip')
    )
...
Test-Case 'Targeted rebuild does not plan when the package already uses the current artifact' {
    $local = @(New-Release -Tag 'pglogical-2.4.8-pg18-windows.2' -ArtifactFilename 'postgresql-18.6-2-windows-x64-binaries.zip')
    $artifacts = [ordered]@{ '18' = New-Artifact -Major '18' -Minor '6' -Revision 2 }
    $results = @(New-Result -Major '18' -PackageTag 'pglogical-2.4.8-pg18-windows.2' -PackageArtifact 'postgresql-18.6-2-windows-x64-binaries.zip' -ServerArtifact 'postgresql-18.6-2-windows-x64-binaries.zip')
    $plan = @(Get-TargetedRebuildPlan -Version '2.4.8' -UpstreamTag 'REL2_4_8' -CommitSha '9a0e182745885ad0152ea387988c95a483396a81' -FailedMajors @('18') -Artifacts $artifacts -LocalReleases $local -CompatibilityResults $results)
    Assert-Equal 0 $plan.Count
}

Test-Case 'Environment and download failures never create a targeted rebuild' {
    $local = @(New-Release -Tag 'pglogical-2.4.8-pg18-windows.1' -ArtifactFilename 'postgresql-18.4-1-windows-x64-binaries.zip')
    $artifacts = [ordered]@{ '18' = New-Artifact -Major '18' -Minor '6' -Revision 2 }
    $results = @(
...
Test-Case 'Compatibility failure marker is deterministic' {
    Assert-Equal '<!-- pglogical-compatibility-failure: pg18/pglogical-2.4.8-pg18-windows.1/postgresql-18.6-2-windows-x64-binaries.zip -->' (Get-CompatibilityFailureMarker -Major '18' -PackageTag 'pglogical-2.4.8-pg18-windows.1' -ServerArtifactFilename 'postgresql-18.6-2-windows-x64-binaries.zip')
}
