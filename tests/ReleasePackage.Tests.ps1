    ...
    switch ($script:ApiMode) {
        'ok' {
            return @{ ExitCode = 0; Stdout = (@{
                id = 123
                tag_name = 'pglogical-2.4.8-pg18-windows.1'
                draft = $false
                prerelease = $false
...
Test-Case 'GitHub release lookup accepts a published JSON response' {
    $script:ApiMode = 'ok'
    $release = Get-GitHubReleaseByTag -Repository 'o/r' -Tag 'pglogical-2.4.8-pg18-windows.1'
    Assert-Equal 123 $release.id
    Assert-Equal 'pglogical-2.4.8-pg18-windows.1' $release.tag_name
}
...
Test-Case 'Release assets require exactly one package ZIP and one checksum file' {
    $assets = @(
        [pscustomobject]@{ name = 'pglogical-2.4.8-pg18.4-edb2-windows-x64.zip'; browser_download_url = 'https://example.invalid/package.zip' },
    ...
    Assert-Equal 'pglogical-2.4.8-pg18.4-edb2-windows-x64.zip' $selected.Package.name
...
Test-Case 'Checksum parser accepts a matching line and rejects mismatch or ambiguity' {
    $name = 'pglogical-2.4.8-pg18.4-edb2-windows-x64.zip'
    $hash = ('a' * 64)
...
try {
    ...
    Test-Case 'Synthetic ZIP expands and exposes the parsed staging/provenance' {
        ...
        Assert-True (Test-ReleasePackageProvenance -Info $archive.BuildInfo -ReleaseTag 'pglogical-2.4.8-pg18-windows.1' -PostgresqlMajor '18' -PackageAssetName 'pglogical-2.4.8-pg18.4-edb2-windows-x64.zip')
    }
    ...
    Test-Case 'Provenance rejects a wrong PostgreSQL major and malformed BUILD-INFO' {
        ...
        Assert-Throws { Test-ReleasePackageProvenance -Info $archive.BuildInfo -ReleaseTag 'pglogical-2.4.8-pg18-windows.1' -PostgresqlMajor '17' -PackageAssetName 'pglogical-2.4.8-pg18.4-edb2-windows-x64.zip' } -MessagePattern 'major'
        $broken = [pscustomobject]@{ pglogicalVersion = '2.4.8' }
        Assert-Throws { Test-ReleasePackageProvenance -Info $broken -ReleaseTag 'pglogical-2.4.8-pg18-windows.1' -PostgresqlMajor '18' -PackageAssetName 'package.zip' } -MessagePattern 'missing'
    }
