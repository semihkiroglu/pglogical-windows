...
Test-Case 'Existing-release detection: HTTP 200 -> exists' {
    $script:FakeResponses = @{ 'repos/o/r/releases/tags/2.4.8-pg18-w1' = @{ Status = 200; Tag = '2.4.8-pg18-w1' } }
    Assert-Equal 'exists' (Test-ExistingRelease -Tag '2.4.8-pg18-w1' -Repository 'o/r')
}

Test-Case 'Existing-release detection: HTTP 404 -> absent' {
    $script:FakeResponses = @{ 'repos/o/r/releases/tags/2.4.8-pg18-w1' = @{ Status = 404; Tag = '' } }
    Assert-Equal 'absent' (Test-ExistingRelease -Tag '2.4.8-pg18-w1' -Repository 'o/r')
}

Test-Case 'Existing-release detection: HTTP 401 -> fail closed' {
    $script:FakeResponses = @{ 'repos/o/r/releases/tags/2.4.8-pg18-w1' = @{ Status = 401; Tag = '' } }
    Assert-Throws { Test-ExistingRelease -Tag '2.4.8-pg18-w1' -Repository 'o/r' } -MessagePattern 'indeterminate'
}

Test-Case 'Existing-release detection: HTTP 403 -> fail closed' {
    $script:FakeResponses = @{ 'repos/o/r/releases/tags/2.4.8-pg18-w1' = @{ Status = 403; Tag = '' } }
    Assert-Throws { Test-ExistingRelease -Tag '2.4.8-pg18-w1' -Repository 'o/r' } -MessagePattern 'indeterminate'
}

Test-Case 'Existing-release detection: HTTP 429 -> fail closed' {
    $script:FakeResponses = @{ 'repos/o/r/releases/tags/2.4.8-pg18-w1' = @{ Status = 429; Tag = '' } }
    Assert-Throws { Test-ExistingRelease -Tag '2.4.8-pg18-w1' -Repository 'o/r' } -MessagePattern 'indeterminate'
}

Test-Case 'Existing-release detection: HTTP 500 -> fail closed' {
    $script:FakeResponses = @{ 'repos/o/r/releases/tags/2.4.8-pg18-w1' = @{ Status = 500; Tag = '' } }
    Assert-Throws { Test-ExistingRelease -Tag '2.4.8-pg18-w1' -Repository 'o/r' } -MessagePattern 'indeterminate'
}
...
Test-Case 'Existing-release detection: malformed 200 response -> fail closed' {
    $script:FakeResponses = @{ 'repos/o/r/releases/tags/2.4.8-pg18-w1' = @{ Status = 200; Tag = '' } }
    # Override stdout with garbage via a custom runner for this case
    $script:GhApiRunner = {
        param([string[]]$Args)
        return @{ ExitCode = 0; Stdout = 'not-a-number'; Stderr = '' }
    }
    Assert-Throws { Test-ExistingRelease -Tag '2.4.8-pg18-w1' -Repository 'o/r' } -MessagePattern 'unexpected response'
    $script:GhApiRunner = 'Invoke-FakeGhApi'
}
...
Test-Case 'Cleanup ownership: job created the draft -> cleanup allowed' {
    $script:FakeResponses = @{
        'repos/o/r/releases/42' = @{ Status = 200; Tag = '2.4.8-pg18-w1' }
    }
    Assert-True (Test-ReleaseOwnership -Created 'true' -ReleaseId '42' -Tag '2.4.8-pg18-w1' -Repository 'o/r')
}

Test-Case 'Cleanup ownership: release existed before the job -> forbidden' {
    $script:FakeResponses = @{ 'repos/o/r/releases/42' = @{ Status = 200; Tag = '2.4.8-pg18-w1' } }
    Assert-False (Test-ReleaseOwnership -Created 'false' -ReleaseId '42' -Tag '2.4.8-pg18-w1' -Repository 'o/r')
}

Test-Case 'Cleanup ownership: created=true but tag identity does not match -> forbidden' {
    $script:FakeResponses = @{
        'repos/o/r/releases/42' = @{ Status = 200; Tag = '2.4.8-pg17-w1' }
    }
    Assert-False (Test-ReleaseOwnership -Created 'true' -ReleaseId '42' -Tag '2.4.8-pg18-w1' -Repository 'o/r')
}
...
Test-Case 'Cleanup ownership: created=true but invalid release ID -> forbidden' {
    Assert-False (Test-ReleaseOwnership -Created 'true' -ReleaseId 'abc' -Tag '2.4.8-pg18-w1' -Repository 'o/r')
}

Test-Case 'Cleanup ownership: creation failed before ID capture -> forbidden' {
    Assert-False (Test-ReleaseOwnership -Created '' -ReleaseId '' -Tag '2.4.8-pg18-w1' -Repository 'o/r')
}
