...
Test-Case 'A brand-new upstream version plans w1 for every missing major' {
    $artifacts = [ordered]@{
    ...
    Assert-Equal '2.4.9-pg14-w1' $plan[0].localTag
...
Test-Case 'Only the missing major is planned; existing majors do not need artifact resolution' {
    ...
    $releases = @(
        (New-Release -Tag '2.4.9-pg14-w1' -Body 'malformed legacy body'),
        (New-Release -Tag '2.4.9-pg18-w1')
    )
...
Test-Case 'Draft and prerelease releases do not count as normal coverage' {
    $draft = New-Release -Tag '2.4.9-pg18-w1'
    $draft.draft = $true
    $pre = New-Release -Tag '2.4.9-pg14-w1'
    $pre.prerelease = $true
    $artifacts = [ordered]@{
        '14' = New-Artifact -Major '14' -Minor '24' -Revision 1
        '18' = New-Artifact -Major '18' -Minor '6' -Revision 2
