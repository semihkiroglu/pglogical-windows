...
function ConvertFrom-LocalReleaseTag {
    ...
    $m = [regex]::Match($Tag.Trim(), '^pglogical-([0-9]+\.[0-9]+\.[0-9]+)-pg([0-9]+)-windows\.([0-9]+)$')
...
function Get-ReleasePlan {
    ...
    .DESCRIPTION
        ...
        windows.1. Compatibility smoke evidence is the only path that may
...
function Test-ReleasePackageProvenance {
    ...
    $expectedAsset = Get-PackageZipName -PglogicalVersion ([string]$Info.pglogicalVersion) -PostgresqlMajor $PostgresqlMajor -PostgresqlMinor $build.Groups[2].Value -EdbPackagingRevision ([int]$Info.edbPackagingRevision)
